...
For release 0.1.2, both the workspace package version and all internal dependency requirements must be `0.1.2`. Leaving an internal requirement at `0.1.1` makes a provider tarball compile against the old registry core, which does not contain the current multimodal, reasoning, tool-signature, and usage contracts.
