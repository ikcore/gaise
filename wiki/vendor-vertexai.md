...
`POST {{MODEL}}:predict` with body `{ "instances": [{ "content": "<text>" }, …], "parameters": { "autoTruncate": true } }` ([`GoogleEmbeddingsRequest::from`](../gaise-provider-vertexai/src/contracts/models.rs#L694)). Each `OneOrMany<String>` input becomes one instance. `task_type`, `title`, `outputDimensionality`, and multimodal instances are not mapped; `autoTruncate` is always `true`. The response `predictions[].embeddings.values` becomes `output: Vec<Vec<f32>>` in order (predictions without `embeddings` are skipped); `metadata` maps to usage as above. `external_id` is `None`. Tier headers are not applied to embeddings.
...
| `gemini-embedding-2` | `gemini-embedding-2-preview` | `active` | — | text, image, audio, video | embedding | embeddings | — | native | GA 2026-04-22 (preview alias since 2026-03-10). Text, image, audio, video, and PDF input; 8,192 input tokens; up to 3,072 dimensions wi… |
| `gemini-embedding-001` | — | `active` | retirement_not_before 2028-05-20 | text | embedding | embeddings | — | native | Previous-generation text embedding model; gemini-embedding-2 is current. |
| `text-embedding-005` | `text-embedding-004`, `text-multilingual-embedding-002`, `multimodalembedding@001` | `active` | retirement_not_before 2027-04-01 | text | embedding | embeddings | — | native for text embeddings | Legacy embedding family; all retire 2027-04-01. |
