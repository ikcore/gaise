...
`GaiseEmbeddingsRequest` carries `task` (`GaiseEmbeddingTask`), `dimensions`, and `normalize`. Map `task` to the provider's document/query distinction (Gemini/Vertex `taskType`, Cohere `input_type`, the `gemini-embedding-2` prompt instruction via `GaiseEmbeddingTask::gemini_instruction`), snap `dimensions` with `snap_dimensions` onto what the model offers, and normalize locally (`normalize_l2`) when the provider cannot — truncated `gemini-embedding-001` vectors are always normalized. Vertex `gemini-embedding-001` takes one text per call. Keep `wiki/embeddings.md` current when a model's limits change.
