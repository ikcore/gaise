...
use gaise_core::contracts::{
    ...
    GaiseInstructStreamResponse, GaiseMessage, GaiseStreamChunk, OneOrMany,
};
...
impl GaiseClient for MockClient {
    ...
    async fn instruct_stream(
        ...
        Box<dyn std::error::Error + Send + Sync>,
    > {
        let chunks = vec![Ok(GaiseInstructStreamResponse {
...
async fn test_logger_integration() {
    ...
    service.instruct(&req).await.unwrap();
    {
        ...
        assert!(
            l.contains(&"REQ: cid=Some(\"cid1\"), type=instruct, model=mock::model".to_string())
        );
        assert!(
            l.contains(&"RES: cid=Some(\"cid1\"), type=instruct, model=mock::model".to_string())
        );
    }
    // Test Stream
    ...
    while stream.next().await.is_some() {}
    {
        let l = logs.lock().unwrap();
        assert!(l.contains(
            &"REQ: cid=Some(\"cid1\"), type=instruct_stream, model=mock::model".to_string()
        ));
        assert!(l.contains(
            &"CHUNK: cid=Some(\"cid1\"), type=instruct_stream, model=mock::model".to_string()
        ));
    }
    // Test Embeddings
    ...
    service.embeddings(&emb_req).await.unwrap();
    {
        let l = logs.lock().unwrap();
        assert!(
            l.contains(&"REQ: cid=Some(\"cid2\"), type=embeddings, model=mock::model".to_string())
        );
        assert!(
            l.contains(&"RES: cid=Some(\"cid2\"), type=embeddings, model=mock::model".to_string())
        );
    }
}
